...
class SingleVariablePrototypesTrainer(torch.nn.Module):
    ...
    def prototype_similarity_penalty(self, data):
        ...
        for i in range(self.num_variables):
            ...
            embeddings = self.wrapper.single_variable_prototype_modules[i].encoder(inp)
    ...
    def plot_all_latent_spaces_with_prototypes(self, use_test=True):
        ...
        for data_matrix, labels in ds:
            for variable in range(self.num_variables):
                ...
                with torch.no_grad():
                    embeddings = self.wrapper.single_variable_prototype_modules[variable].encoder(inp)
    ...
    def evaluate(self, use_test=True):
        with torch.no_grad():
            numerator = 0
            denominator = 0
            for train, label in self.test_dataloader:
                _, classification_output = self.wrapper(train.float())
