...
if __name__ == "__main__":
    ...
    loop = MultivariablePrototypeLearningTrainLoopReal(6,10,4,4,None)
    sequence = PretrainAndSaveLoop(loop)
    ...
    with torch.no_grad():
        ...
        for train_sample  in visualize_moment:
            ...
            for var in range(6):
                ...
                out = torch.concat([out, 4*torch.ones((loop.framework.signal_prototypes[var].protolayer.prototype_matrix.shape[0],))],dim=0)
                visualizer = UMAPLatent()
                visualizer.visualize(embeddings,out, 4)
    
    loop2 = MultivariablePrototypeLearningTrainLoopSecond(loop.framework.signal_prototypes,6,24,4,4)
