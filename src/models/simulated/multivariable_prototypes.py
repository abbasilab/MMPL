...
if __name__ == "__main__":
    ...
    sns.heatmap(torch.relu(model.aggregate_prototype_layer.protos[:, :-4]).detach().numpy())
    plt.show()
    choice = input()
    if choice == "n":
        exit()
    ...
    for epoch in tqdm(range(epochs)):
        for train, label in data_train:
            ...
            # print(float(similarity_penalty1(second_degree, model.aggregate_prototype_layer.protos)),
            #        float(similarity_penalty3(second_degree, model.aggregate_prototype_layer.protos)),
            #          float(diversity_penalty(model.aggregate_prototype_layer.protos)))
